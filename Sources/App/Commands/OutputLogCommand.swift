...
struct OutputLogCommand: AsyncCommand {
  ...
  func run(using context: CommandContext, signature: Signature) async throws {
    ...
    for measure in measures {
      context.console.print("| \(measure.createdAt!.ISO8601Format()) | \(measure.cpuPercent) | \(measure.memoryMB) | \(measure.way.rawValue) | \(measure.chunk.map(String.init) ?? "nil") | \(measure.start.ISO8601Format()) | \(measure.end.ISO8601Format()) | \(measure.clientTime) | \(measure.serverTime) | \(measure.userEventCount) | \(measure.allEventCount) | \(measure.enableIndex) | \(measure.point) |")
    }
  }
}
